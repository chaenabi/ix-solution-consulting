...
const deletePost = e => {
  if (confirm('정말로 삭제하시겠습니까? 삭제하면 되돌릴 수 없습니다.')) {
    ...
    axios
      .post(`http://localhost:8080/v1/posts/remove`, body, {
        headers: {
...
const addComment = () => {
  ...
  const result = axios.post(`http:/localhost:8080/v1/comments`, body)
...
// 3.39.207.182:8080
  const result = axios.patch(`http://localhost:8080/v1/comments`, body)
...
const deleteComment = e => {
  ...
  const result = axios.delete(`http://localhost:8080/v1/comments/remove`, {
    params,
  })
